                    ...
                    <template slot="body" slot-scope="props">
                        ...
                        <td class="rule-table-editor">
                            <div style="margin: 0 10px;">
                                <span
                                v-if="parseInt(props.row.question_type) === 1">
                                    {{props.row.ratio}}
                                </span>
                                <el-input-number v-model="props.row.ratio" placeholder="请输入内容"
                                v-else
                                @blur="vaildNumber1(props.row.ratio, rule1Data, props.index, 'ratio')"
                                :min="0"
                        ...
                        <td class="rule-table-editor">
                            <div style="margin: 0 10px;">
                                ...
                                <input type="text" class="el-input__inner" v-model="props.row.read_time" placeholder="请输入数字" style="max-width: 100px;text-align:center;"
                                v-else
                                @input="() => {vaildReadTIme(props.row.read_time, rule1Data, props.index, 'read_time')}"
                                >
                                <!-- <el-input-number v-model="props.row.read_time" placeholder="请输入内容"
                                v-else
                                @input="vaildReadTIme"
                                @blur="vaildNumber1(props.row.read_time, rule1Data, props.index, 'read_time')"
                                :min="0"
            ...
            :data="rule2Data">
                ...
                <template slot="body" slot-scope="props">
                    ...
                    <td class="rule-table-editor">
                        <div style="margin: 0 10px;">
                            <span
                            v-if="parseInt(props.row.ratio) === 0">
                                {{props.row.deviation}}
                            </span>
                            <el-input-number v-model="props.row.deviation" placeholder="请输入内容"
                            v-else
                            @blur="vaildNumber2(props.row.deviation, rule2Data, props.index, 'deviation')"                            
                            :min="0"
                            :max="parseInt(props.row.score)"></el-input-number>
                        </div>
                    </td>
...
export default {
    ...
    methods: {
        ...
        // 最短评阅时间验证
        vaildReadTIme (value, data, index, key) {
            let t = Number(value)
            if (isNaN(t)) {
                let parseT = ''
            ...
            if (t < 0) {
                this.$message.warning('最短批阅时间不能小于0秒')
                t = 0
            } else if (t > 60) {
                this.$message.warning('最短批阅时间不能大于60秒')
                t = 60
            }
            let row = {}
        ...
        vaildNumber1 (value, data, index, key) {
            let row = {}
            Object.assign(row, data[index], {
                [key]: parseInt(value)
            })
        ...
        vaildNumber2 (value, data, index, key) {
            let row = {}
            Object.assign(row, data[index], {
                [key]: parseInt(value * 10) / 10
            })
            this.$set(data, index, row)
        },
    }
