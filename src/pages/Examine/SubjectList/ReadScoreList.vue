            ...
            @sort-change="handleSortChange">
                ...
                label="班级">
                    <template slot-scope="scope">
                        {{scope.row.grade + scope.row.class}}
                    </template>
                </el-table-column>
                <el-table-column
                sortable
        ...
        req () {
            ...
            .then(res => {
                this.tableData = res.data.row.map(item => {
                    item.class_rank = parseInt(item.class_rank)
                    item.subj_score = parseInt(item.subj_score)
                    item.obje_score = parseInt(item.obje_score)
                    item.score = parseInt(item.score)
                    item.grade_rank = parseInt(item.grade_rank)
                    return item
                })
