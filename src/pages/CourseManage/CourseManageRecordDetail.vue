        ...
        @update="init">
            <template slot="head">
                ...
                <el-row>
                    ...
                    <el-select  v-if="qryForm.qryType == 2" filterable  v-model="qryForm.qryClassId" placeholder="请选择班级" style="width: 202px;" :disabled="btnType==2">
            ...
            <template slot="content" slot-scope="props">
                    ...
                    :data="tableData">
                        ...
                        <el-table-column prop="col7"  label="星期日">
                            <template slot-scope="scope">
                                 <div v-if="btnType==1">{{scope.row.col7}}</div>
                                <div v-if="btnType==2 && scope.row.col7.subject_id==''">暂无排课</div>
                                <div v-if="btnType==2 && scope.row.col7.subject_id!=''">
                                    <el-select v-model="scope.row.col7.subject_id"  placeholder="请选择科目" @change="selectChangedForSubjectCol(scope.row.col7)">
                                        <el-option v-for="(item,index) in scope.row.col7.subjectList"
                                            :key="index"
                                            :label="item.label"
                                            :value="item.value">
                                        </el-option>
                                    </el-select>
                                    <label v-if="scope.row.col7.subject_id!=''">{{scope.row.col7.teacherName}}</label>
                                </div>
                            </template>
                        </el-table-column>
                    </el-table>
...
export default {
    ...
    methods: {
        init(){
            this.qryForm.qryClassId = "";
            this.qryForm.qryTeacherName="";
            this.tableData = [];
            //查询当前登录者的教师ID及姓名
        ...
        btnClickForQueryByTeacher(){
            ...
            this.$http.post('/TimeTable/index', {
                ...
                res.data.forEach(row=>{
                    let rowObj = {time:"",section:"",col1:"",col2:"",col3:"",col4:"",col5:"",col6:"",col7:"",rowspan:1,colspan:1,spanIndex:0};
