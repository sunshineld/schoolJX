        ...
        @update="init">
            <template slot="head">
                ...
                <el-row style="margin-top:8px;">
                    ...
                    <label>排课年级</label>
                    <el-select v-model="selForm.gradeId" filterable default-first-option  placeholder="请选择排课年级" @change="selectChangeForGrade" style="width: 180px;">
                        <el-option v-for="item in gradeList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                    <label>课表时间</label>
                    <el-select v-model="selForm.dateId" filterable default-first-option placeholder="请选择课表时间" @change="selectChangeForGradeTime" style="width: 180px;">
...
export default {
  ...
  methods: {
    ...
    selectChangeForGrade(){
        ...
        .then(res => {
            ...
            if(res.data.length<1){
                this.$message.error("请先完成教学任务分配再进行排课！");
                this.$store.commit('startLoad');
                setTimeout(() => {
                    this.$store.commit('endLoad');
                    this.$router.push({ path: "/teaching-manage-item" });      
                }, 3000);
                return;
            }
            res.data.forEach(e=>{
    ...
    selectChangeForGradeTime(){
        ...
        .then(res => {
            if(!res || !res.data || !res.data.node || res.data.node.length<1){
                this.$message.error("请先完成课表时间设置后再进行排课！");
                this.$store.commit('startLoad');
                setTimeout(() => {
                    this.$store.commit('endLoad');
                    this.$router.push({ path: "/course-manage-time-record" });      
                }, 3000);
                this.tableData=[];
                return;
            }
