            ...
            <template slot="content" slot-scope="props">
                    ...
                    @sort-change="sortChange">
                        ...
                        <template slot-scope="scope">
                            <el-button @click="btnClickForEdit(scope.row)" type="text" size="small" v-if="scope.row.is_use=='0'">修改</el-button>
                            <el-button type="text" size="small" v-if="scope.row.is_use=='1'" disabled  style="background:#ccc">修改</el-button>
                            <el-button @click="btnClickForDelDialog(scope.row)" type="text" size="small" v-if="scope.row.is_use=='0'">删除</el-button>
                            <el-button type="text" size="small" v-if="scope.row.is_use=='1'" disabled  style="background:#ccc">删除</el-button>
                        </template>
                        </el-table-column>
                    </el-table>
                <!-- 新增 -->
                <I-table-editor v-model="dialogFormVisible" title="添加课表时间设置" width="530px">
                    <template slot="content">
                        ...
                        <el-row>
                            <label>标题：</label>
                            <el-input v-model="addForm.title" auto-complete="off" :maxlength=length1 placeholder="请输入标题" style="width:440px;"></el-input>
                        </el-row>
                        <el-row>
                            <el-table
                                border
                                ...
                                <el-table-column label="节次名" disabled >
                                    <template slot-scope="scope">
                                        <el-input v-model="scope.row.namevalue" auto-complete="off" :maxlength=length2 placeholder="请输入节次名"></el-input>
...
export default {
  ...
  methods: {
    ...
    doSearch() {
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (this.current > 1 && this.tableData.length == 0) {
            this.current--;
            this.doSearch();
          }
        });
    ...
    btnClickForEdit(obj) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          // 根据id排序 解决因服务器返回数组顺序不同
          let nodeMap = {};
          for (let i = 0, maxI = res.data.node.length; i < maxI; ++i) {
            let item = res.data.node[i];
            nodeMap[item.id] = item;
          }
          res.data.node = [];
          for (let ii of Object.keys(nodeMap)) {
            res.data.node.push(nodeMap[ii]);
          }
          //分拆node - 返回数据变态
          var node1 = [],
            node2 = [],
          ...
          for (let k = 0; k < res.data.node.length; k++) {
            ...
            if (item.node_cate == "1") {
              node1.push({ id: 1, namevalue: item.node, tagvalue: 1 });
            }
            if (item.node_cate == "2") {
              node2.push({ id: 1, namevalue: item.node, tagvalue: 2 });
            }
            if (item.node_cate == "3") {
              node3.push({ id: 1, namevalue: item.node, tagvalue: 3 });
            }
            if (item.node_cate == "4") {
              node4.push({ id: 1, namevalue: item.node, tagvalue: 4 });
            }
          }
          this.addForm.dataList = Array.concat(node1, node2, node3, node4);
          for (let k = 0; k < this.addForm.dataList.length; k++) {
            this.addForm.dataList[k].id = k + 1;
          }
    ...
    btnClickForDelDialog(obj) {
      if (obj == null) {
        this.$message.error("您选择的信息不存在或已被删除，请刷新后重试！");
        return;
      }
    ...
    //弹出层-确定
    btnClickForDoYes() {
      if (this.waitForDoType == 1) {
        this.btnClickForAddYes();
        return;
      }
      if (this.waitForDoType == 2) {
        this.btnClickForEditYes();
        return;
      }
    },
    //添加  - 确定
    btnClickForAddYes() {
      ...
      for (let k = 0; k < this.addForm.dataList.length; k++) {
        if (this.addForm.dataList[k].namevalue == "") {
          this.$message.error("课节名称不能为空");
          return;
        }
      ...
      //调取接口-提交-返回
      this.$http.post("/Course/timeAdd", submitObj).then(res => {
        //回调后
    ...
    btnClickForEditYes() {
      ...
      submitObj.node = JSON.stringify(submitObj.node);
      //调取接口-提交-返回
      this.$http.post("/Course/timeEdit", submitObj).then(res => {
