    ...
    <div>
        ...
        @update="doSearch">
            <template slot="head">
                ...
                <!-- <el-button size="smail" type="primary" class="confirm btnBatch" @click="btnClickForBatchDel" >批 量 删 除</el-button> -->
            </template>
            <template slot="content" slot-scope="props">
                    ...
                    <template slot="content">
                        <el-form>
                            <el-form-item label="课表名称:" :label-width="formLabelWidth">
                                <el-input v-model="copyForm.targetName" :maxlength=length01 auto-complete="off" style="width:93%;" placeholder="请输入课表名称"></el-input>
                            </el-form-item>
                        </el-form>
                    </template>
                    <template slot="foot">
                ...
                title="删除">
                    <template slot="content">
                        <div class="outportaddr-group">删除之后不可恢复，确认进行删除？</div>
                    </template>
                    <template slot="foot">
                        <el-button 
            ...
            <template slot="foot">
                    <el-button type="danger" style="margin: 0 10px;"  @click="btnClickForBatchDel">批量删除</el-button>
                    <I-table-page  :total="total" @change="pageChange"></I-table-page>
            </template>
        </I-table>
    </div>
...
export default {
  ...
  methods: {
    doSearch() {
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (this.tableData.length < 1 && this.pageIndex > 0) {
            this.pageIndex--;
            this.doSearch();
          }
        });
    ...
    //批量删除 - 进入
    btnClickForBatchDel() {
      if (this.waitForDelIdList.length <= 0) {
        this.$message.error("请选择课程"); //个人觉得“请先勾选待删除课程后再执行此操作！” 会更好。应产品和测试要求吧。
        return;
      }
    ...
    btnClickForCopyAndUsedYes() {
      ...
      if (this.copyForm.targetName.length < 2) {
        this.$message.error("请输入课表名称，至少2个字符！");
        return;
      }
