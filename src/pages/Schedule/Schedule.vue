    ...
    <div>
        ...
        @update="init">
            <template slot="head">
                ...
                @select="handleSelect"
                ></el-autocomplete>
                <el-select @change="searchClass" filterable v-model="classId" placeholder="请选择班级" v-if="radio == 2">
                    <el-option
...
export default {
  ...
  methods: {
    ...
    init() {
      this.tableData = [];
      this.classId = "";
      this.queryString = localStorage.jxname;
      this.req();
    },
    //点击查询
    searchFn() {
          ...
          .then(res => {
            ...
            if (res.data.length > 0) {
              for (let i = 0; i < res.data.length; i++) {
                ...
                this.teacherIdData[i].value = res.data[i].name + ' ' + res.data[i].phone;
      ...
      this.$http
        ...
        .then(res => {
          ...
          for (let i = 0; i < res.data.row.length; i++) {
            ...
            this.classOptions[i].label =
              res.data.row[i].grade + res.data.row[i].class;
          }
        })
        .catch(res => {
          this.$message.error(res.info);
        ...
        .then(res => {
          ...
          res.data.forEach(row => {
            ...
            for (let index in row) {
              ...
              if (index == 1) {
                rowObj.time = colItem.type;
                rowObj.section = colItem.section;
              }
              rowObj["col" + index] =
                colItem.subject_name +
                " " +
                (colItem.class == undefined || colItem.class == null
                  ? ""
                  : colItem.class);
          ...
          for (let k = 0; k < node1.length; k++) {
            ...
            element.spanIndex = k;
            if (k == 0) {
              element.rowspan = node1.length;
            } else {
              element.time = element.section;
            }
          }
