              ...
              <div v-for="(site,indexA) in bigJson" :key="indexA" class="items">	
                <div v-for="(bigli,indexB) in site.content" :key="indexB">
                      ...
                      @mousedown="draggablefn($event)" @mouseup="draggablefn2($event)">         	       	
		                  <div class="left">
                        <input v-if="smallli.point == null" maxlength="3" class="setScore" type="text" placeholder="设分" @change="toScore(indexA,indexB,indexC,$event)" @mousedown="drag1($event)">
		                    <input v-else class="setScore" maxlength="3" :value="smallli.point" type="text" placeholder="设分" @change="toScore(indexA,indexB,indexC,$event)" @mousedown="drag1($event)"><br>
...
export default {
  ...
  methods: {
    ...
    deleteconfirm() {
      ...
      if (
        this.bigJson[this.placeA].content[this.placeB].smContent[this.placeC]
          .question_type == 4 ||
        !this.bigJson[this.placeA].content[this.placeB].smContent[this.placeC]
          .question_type
      ) {
        if (
          getData.selected[0].indexOf(
            this.bigJson[this.placeA].content[this.placeB].smContent[
              this.placeC
            ].id
          ) != -1
        ) {
          getData.selected[0].splice(
            getData.selected[0].indexOf(
              this.bigJson[this.placeA].content[this.placeB].smContent[
                this.placeC
              ].id
      ...
      if (
        ...
        if (
          getData.selected[1].indexOf(
            this.bigJson[this.placeA].content[this.placeB].smContent[
              this.placeC
            ].id
          ) != -1
        ) {
          getData.selected[1].splice(
            getData.selected[1].indexOf(
              this.bigJson[this.placeA].content[this.placeB].smContent[
                this.placeC
              ].id
      ...
      if (
        ...
        if (
          getData.selected[2].indexOf(
            this.bigJson[this.placeA].content[this.placeB].smContent[
              this.placeC
            ].id
          ) != -1
        ) {
          getData.selected[2].splice(
            getData.selected[2].indexOf(
              this.bigJson[this.placeA].content[this.placeB].smContent[
                this.placeC
              ].id
      ...
      if (
        ...
        if (
          getData.selected[3].indexOf(
            this.bigJson[this.placeA].content[this.placeB].smContent[
              this.placeC
            ].id
          ) != -1
        ) {
          getData.selected[3].splice(
            getData.selected[3].indexOf(
              this.bigJson[this.placeA].content[this.placeB].smContent[
                this.placeC
              ].id
      ...
      for (var z = 0; z < questions.length; z++) {
        if (
          this.bigJson[this.placeA].content[this.placeB].smContent[this.placeC]
            .id == questions[z].id &&
          this.bigJson[this.placeA].content[this.placeB].smContent[this.placeC]
            .question_type == questions[z].question_type
        ) {
      ...
      this.bigJson[this.placeA].content[this.placeB].smContent.splice(
        this.placeC,
        1
      );
      if (
        this.bigJson[this.placeA].content[this.placeB].smContent.length == 0
      ) {
        this.bigJson[this.placeA].content[this.placeB].smContent = [];
    ...
    submitFn(val) {
      ...
      var papers_type = this.$route.query.papers_type;
      if (
        this.exam_question_list.total_points > 300 ||
        isNaN(this.exam_question_list.total_points) ||
        !!!this.exam_question_list.total_points ||
      ...
      for (var d = 0; d < document.querySelectorAll(".setScore").length; d++) {
        ...
        document.querySelectorAll(".setScore")[d].style.border =
          "1px solid gray";
        if (document.querySelectorAll(".setScore")[d].value == "") {
          document.querySelectorAll(".setScore")[d].style.border =
            "1px solid red";
          document.querySelectorAll(".setScore")[d].focus();
          if ((pointFlag = true)) {
            document.querySelector(".contentCover").scrollTop =
              document.querySelectorAll(".setScore")[d].offsetTop -
              document.querySelector(".contentCover").offsetTop;
            pointFlag = false;
          }
        }
        if (smallScore2) {
      ...
      //提交
      if (val == "finsh") {
        console.log("看看你是啥", papers_type);
        this.$http
          .post("/ExamPaper/savePaperContent", {
          ...
          .then(res => {
            ...
            this.$router.push("/papers/" + papers_type + "?type=" + type);
            // history.go(-2);
          })
          .catch(res => {
            this.$message.error(res.info);
    ...
    drop: function(ev, type, arr, indexA, indexB, indexC) {
      ...
      if (this.startDiv != this.endDiv) {
        ...
        //试题标号
        var numbers1 = this.bigJson[indexA].content[indexB].smContent[
          this.startIndex
        ].number;
        var numbers2 = this.bigJson[indexA].content[indexB].smContent[
          this.endIndex
        ].number;

        this.bigJson[indexA].content[indexB].smContent[
          this.startIndex
        ] = this.bigJson[indexA].content[indexB].smContent[this.endIndex];
    ...
    toScore: function(indexA, indexB, indexC, event) {
      ...
      if (smallScore > this.exam_question_list.total_points) {
        this.$message.error("输入的分数不能大于试卷总分数");
      ...
      for (var k = 0; k < testdata.length; k++) {
        ...
        for (var m = 0; m < testdata[k].content.length; m++) {
          ...
          for (var z = 0; z < testdata[k].content[m].smContent.length; z++) {
            ...
            console.log(
              "testnumber",
              testdata[k].content[m].smContent[z].number
            );
