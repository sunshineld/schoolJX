<template>
  <div class="papers">
    ...
    @update="refresh">
        <template slot="head">
          <label class="titles">结构：</label>
                <el-radio v-model="mainStructure_type" label="1" @change="mainPoint('1')">按章节</el-radio>
                <el-radio v-model="mainStructure_type" label="2" @change="mainPoint('2')">按知识点</el-radio>
              <el-select clearable v-model="getData.versions_id" @change="changeSelect('version')" placeholder="请选择版本" v-if="mainStructure_type === '1'">
                  <el-option
              ...
              <el-select clearable v-model="getData.classes" @change="changeSelect('class')" placeholder="请选择年级" v-if="mainStructure_type === '1'">
                  <el-option
              ...
              <el-select clearable v-model="getData.subject" placeholder="请选择科目" v-if="mainStructure_type === '1'">
                  <el-option
              ...
              <el-select clearable v-model="getData.pharse_id" @change="changeSelect('pharse')" placeholder="请选择学阶"  v-if="mainStructure_type === '2'">
                  <el-option
                  v-for="(item,index) in getPharseData"
                  :key="index"
                  :label="item.pharse_name"
                  :value="item.id">
                  </el-option>
              </el-select> 
              <el-select clearable v-model="getData.classes" @change="changeSelect('class')" placeholder="请选择年级"  v-if="mainStructure_type === '2'">
                  <el-option
                  v-for="(item,index) in gradeData"
                  :key="index"
                  :label="item.class"
              ...
              <el-select clearable v-model="getData.subject" placeholder="请选择科目"  v-if="mainStructure_type === '2'">
                  <el-option
                  v-for="item in sujectData"
                  :key="item.value"
                  :label="item.subject"
              ...
              <el-input v-model="getData.keyword" size="smail" placeholder="请输入试卷标题" style="width:150px;"></el-input>
          ...
          :title="showTitle">
            <template slot="content">
              <div class="list">
                ...
                <label class="titles">结构：</label>
                <el-radio v-model="addInfo.structure_type" label="1" @change="point('1')" :disabled="!edit_is">按章节</el-radio>
                <el-radio v-model="addInfo.structure_type" label="2" @change="point('2')" :disabled="!edit_is">按知识点</el-radio>
              </div>
              <div class="list" v-if="addInfo.structure_type == 1">
              ...
              <div class="list" v-if="addInfo.structure_type == 2">
                <label class="titles">学阶：</label>
                <el-select @change="addChangeSelect('pharse')" v-model="addInfo.pharse_id" placeholder="请选择学阶" :disabled="!edit_is">
                  <el-option
                    v-for="(item,index) in getPharseData"
                    :key="index"
                    :label="item.pharse_name"
              ...
              <div class="list">
                ...
                <input type="text" maxlength="3" v-model="addInfo.total_points" placeholder="分数" :disabled="!edit_is">
                <label class="titles">考试时间：</label>
                <input type="text" maxlength="3" v-model="addInfo.exam_minutes" placeholder="分钟">
              </div>
            </template>
            <template slot="foot">
                <el-button 
...
export default {
  ...
  mounted() {
    ...
    if(this.type) {
      this.mainStructure_type = this.type;
    }
    console.log("类型。。。。",this.mainStructure_type)
    // 
  ...
  watch: {
    $route(to, from) {
      ...
      this.getData.pharse_id = "";
      this.getData.structure_type = this.mainStructure_type;
      this.mainStructure_type = "1";
      this.gradeData = [];
      this.sujectData = [];
  ...
  methods: {
    ...
    mainPoint(val) {
      //是按章节
      this.addInfo.structure_type = val;
      this.getData.structure_type = val;
      this.mainStructure_type = val;
      this.getData.versions_id = "";
      this.getData.clid = "";
      this.getData.subject = "";
      this.getData.keyword = "";
    ...
    getPharse() {
      this.$http
        .post("./Question/getPharse", { token: localStorage.jxtoken })
        .then(res => {
          // console.log(res);
          this.getPharseData = res.data;
        })
        .catch(res => {
    ...
    changeSelect(name, val) {
      ...
      //如果是学阶
      if (name === "pharse") {
        this.getClass("gradeData", this.getData.pharse_id);
        this.getData.class_id = "";
        this.getData.classes = "";
        this.getData.subject = "";
        this.sujectData = [];
    ...
    addChangeSelect(name, val) {
      ...
      if (name == "pharse") {
        this.getClass("addGradeData", this.addInfo.pharse_id);
        this.addInfo.sid = "";
        this.addInfo.clid = "";
        this.addSujectData = [];
      }
    ...
    getClass(data, version_id) {
      ...
      this.$http
        .post("./Question/getClass", {
          ...
          structure_type: this.addInfo.structure_type,
          pharse_id: version_id
        })
        .then(res => {
          //console.log(res);
          this[data] = [];
    ...
    handleClick(val) {
      ...
      this.addInfo.structure_type = val.structure_type;
      this.addInfo.pharse_id = val.pharse_id;

      this.isAdd = false;
      if (val.structure_type == 1) {
        ...
        this.getPharse();
        this.getClass("addGradeData", val.pharse_id);
        this.getSubject("addSujectData", val.pharse_id, val.clid);
      }
    ...
    //添加试题
    addfn() {
      if (this.addInfo.structure_type == 1 && this.addInfo.versions_id == "") {
        this.$message.error("请选择版本!");
        return;
      }
      if (this.addInfo.structure_type == 2 && this.addInfo.pharse_id == "") {
        this.$message.error("请选择学阶!");
        return;
      }
    ...
    addQestions(val) {
      ...
      localStorage.papers_id = val.id;
      localStorage.structure_type = val.structure_type;
      localStorage.pharse_title = val.pharse_title;
      localStorage.pharse_id = val.pharse_id;
      localStorage.add_questions = 1;
      localStorage.removeItem("getData");
      localStorage.removeItem("questions");
      localStorage.removeItem("bigJson");
      console.log("val", val);
      if (val.sname == "英语" || val.sname == "语文") {
        this.$router.push("/papers-add2?type=" + this.mainStructure_type + "&papers_type=" + this.getData.type);
      } else {
        this.$router.push("/papers-add?type=" + this.mainStructure_type + "&papers_type=" + this.getData.type);
      }
    },
    //预览
    priview(val) {
