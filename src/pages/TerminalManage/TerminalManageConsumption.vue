        ...
        title="设置">
            <template slot="content">
                <div class="top">
                    ...
                    <div>设定个人当月营养餐总次数：
                        <el-input v-model="settingData.month_times_total" :minlength="1" :maxlength="3" @keyup.native="keyUpForMonthTotal" size="smail" placeholder="" style="width:70px;"></el-input> 次
                    </div>
                    <div>
                        选择个人当前月营养餐扣除方式:
                ...
                <div class="bottom">
                    ...
                    <div>设定个人当天消费总额：
                        <el-input v-model="settingData.offline_day_money_total" :minlength="1" :maxlength="4"  @keyup.native="keyUpForOfflineTotal" size="smail" placeholder="" style="width:70px;"></el-input> 元
                    </div>
                    <!-- <div>
                        选择个人当前天营养餐消费次数:
...
export default {
  ...
  destroyed() {
    ...
    this.$http
      .post(
        "/device/findOnOff",
        { on_off: 2, token: this.$tools.getToken() },
  ...
  methods: {
    init() {
      ...
      this.$http
        .post(
          "/device/findActiveList",
          { type: 1, token: this.$tools.getToken() },
          { showError: false, showLoading: false }
        )
        .then(res => {
          res.data.forEach(e => {
            this.activeList.push({
              ...
              modify_time: this.$tools.formatDate(e.modify_time)
            });
          });
    ...
    findfn() {
      ...
      this.$http
        ...
        .then(res => {
          ...
          //60秒后，自动停止
          setTimeout(() => {
            if (this.finding) this.unfindfn();
          }, this.cmdTimerTimeout);
          this.cmdTimer = setInterval(() => {
            ...
            this.$http
              ...
              .then(res => {
                ...
                this.datas = this.unactiveList.concat(this.activeList);
              })
              .catch(res => {
                this.activeList = [];
                this.datas = this.unactiveList.concat(this.activeList);
            ...
            this.$http
              .post(
                "/device/findUnActiveList",
                { type: 1, token: this.$tools.getToken() },
                { showError: false, showLoading: false }
              )
              .then(res => {
                res.data.forEach(e => {
                  e.modify_time = this.$tools.formatDate(e.modify_time);
                });
    ...
    btnClickForActive(item) {
      this.$http
        .post(
          "/device/active ",
          { device_id: item.id, token: this.$tools.getToken() },
    ...
    btnClickForDelYes() {
      ...
      this.$http
        .post(
          "/device/delete ",
          { device_id: this.waitForDelID, token: this.$tools.getToken() },
          { showError: true, showLoading: true }
        )
        .then(res => {
          //遍历 - 删除
    ...
    keyUpForMonthTotal() {
      //正则表达式替换，保留正整数
      let str = this.settingData.month_times_total;
      str = str.toString().replace(/[^\d，,]]*/g, "");
      // if(str>100)str=100;
      this.settingData.month_times_total = str;
    },
    //天总额校验
    keyUpForOfflineTotal() {
      //正则表达式替换，保留正整数或1位小数
      let str = this.settingData.offline_day_money_total;
      str = str.toString().replace(/^\D*([0-9]\d*\.?\d{0,1})?.*$/, "$1");
      // if(str>100)str=100;
      // console.log(str);
      this.settingData.offline_day_money_total = str;
    },
    //消费设置 - 确定
    btnClickForSettingYes() {
      if (isNaN(Number(this.settingData.month_times_total))) {
        this.$message.error("当月营养餐总次数仅能输入正整数!");
        return;
      }
      if (this.settingData.month_times_total > 100) {
        this.$message.error("当月营养餐总次数不能超过100次!");
        return;
      }
      if (isNaN(Number(this.settingData.offline_day_money_total))) {
        this.$message.error("离线消费个人当天消费总额仅能输入数字!");
        return;
      }
      if (this.settingData.offline_day_money_total > 100) {
        this.$message.error("离线消费个人当天消费总额不能超过100元!");
        return;
      }
      this.$http.post("/device/paySet", this.settingData).then(res => {
