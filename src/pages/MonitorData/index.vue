                ...
                :title="editTitle">
                    <template slot="content">
                        <el-form >
                            ...
                            <el-input :maxlength="30" v-model="form.name" auto-complete="off" placeholder="请输入监控名称" :clearable="true"></el-input>
                            </el-form-item>
                            <el-form-item label="班级名称:" :label-width="formLabelWidth">
                                <el-select v-model="form.class_id" placeholder="请选择班级名称">
                                    <el-option
                            ...
                            <el-input v-model="form.pc_url" auto-complete="off" placeholder="请输入URL地址" :clearable="true"></el-input>
                            </el-form-item>
                            <el-form-item label="APP地址" :label-width="formLabelWidth">
                            <el-input v-model="form.app_url" auto-complete="off" placeholder="请输入APP地址" :clearable="true"></el-input>
                            </el-form-item>
  ...
  methods: {
    ...
    req() {
      ...
      this.$http
        ...
        .then(res => {
          console.log(res);
          if (this.page_number > 1 && res.data.rows.length < 1) {
    ...
    deleteconfirm() {
      ...
      this.$http
        ...
        .then(res => {
          this.$message.success(res.info || "删除成功");
    ...
    addfn() {
      ...
      if (
        this.form.pc_url.trim() == "" ||
        this.form.pc_url.toString().trim() == ""
      ) {
      ...
      //提交
      if (this.isAdd) {
        this.$http.post("/Monitor/add", this.form).then(res => {
          this.$message.success(res.info);
          this.dialogFormVisible = false;
          //判断是否超过本页数据，若超过，则页面序号+1 提高体验感
          //add by xing.wei with 2018-03-15
          //*********** add by xing.wei with 2018-03-15 start*/
          if (this.tableData.length >= this.page_size) {
            this.page_number = this.total / this.page_size + 1;
            this.total += 1;
          }
          //********************************** end */
          this.req();
        ...
        this.$http.post("/Monitor/edit", this.form).then(res => {
          this.$message.success(res.info);
          this.dialogFormVisible = false;
          this.req();
        });
