<template>
    <div>
        ...
        >
            <template slot="head">
                <div>
                    <el-button size="medium" type="success" class="addbtn confirm" @click="addShow">新 增 规 则</el-button>
                </div>
            </template>
	                   ...
	                   <template slot="content">
	                   		<el-form >
	                            <el-form-item label="规则名称:" :label-width="formLabelWidth">
	                                <el-input v-model="form.name" :aria-modal="20" auto-complete="off" placeholder="请输入规则名称"  :clearable="true"></el-input>
	                            </el-form-item>
		                        <el-form-item label="生效时间:" :label-width="formLabelWidth">
								    <el-col :span="11">
								      <el-date-picker @change="startDatetimeChange" :picker-options="startDatetimePickerOptions" :disabled="istrue"
                                       type="date" placeholder="请选择生效开始日期" v-model="form.effective_start_time" style="width: 100%;"></el-date-picker>
								    </el-col>
								    <el-col class="line" :span="2" style='text-align: center;'>至</el-col>
								    <el-col :span="11">
								      <el-date-picker type="date" @change="endDatetimeChange" :picker-options="endDateTimePickerOptions" :disabled="istrue"
                                       placeholder="请选择生效截至日期" v-model="form.effective_end_time" style="width: 100%;"></el-date-picker>
								    </el-col>
								</el-form-item>
  ...
  methods: {
    ...
    editShow(val) {
      ...
      this.form.effective_start_time = this.$tools.formatDate(
        val.effective_start_time.toString(),
        "yyyy-MM-dd"
      );
      this.form.effective_end_time = this.$tools.formatDate(
        val.effective_end_time.toString(),
        "yyyy-MM-dd"
      );
    ...
    addfn() {
      ...
      if (this.form.name.length < 2) {
        this.$message.error("名称长度为2到10个字符!");
        return;
      }
      ...
      this.form.repeat = this.form.repeat
        .sort((a, b) => {
          return a - b;
        })
        .join(",");
