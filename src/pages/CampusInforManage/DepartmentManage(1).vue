	                   ...
	                   <template slot="content">
	                   		<el-form >
	                            <el-form-item label="科室名称:" :label-width="formLabelWidth">
	                                <el-input v-model="form.name" auto-complete="off" placeholder="请输入科室名称"></el-input>
	                            </el-form-item>
	                            <el-form-item label="位  置:" :label-width="formLabelWidth" class="text-style">
	                                <el-input v-model="form.position" auto-complete="off" placeholder="请输入位置"></el-input>
	                            </el-form-item>
	                            <el-form-item label="用  途:" :label-width="formLabelWidth">
	                                <el-input v-model="form.use" auto-complete="off" placeholder="请输入用途"></el-input>
	                            </el-form-item>
	                        </el-form>
	                    </template>
	                    <template slot="foot">
...
export default {
    ...
    methods: {
        ...
        //确定增加科室
        addDept() {
            let rule = /^[0-9a-zA-Z\u4E00-\u9FA5]{2,20}$/;
            let positionRule = /^[a-zA-Z\u4E00-\u9FA5]{2,20}$/;
            let useRule = /^[a-zA-Z\u4E00-\u9FA5]{2,10}$/;
        	if(this.form.name.trim() == ""){
        		this.$message.error('请输入科室名称!'); 
        		return;
            }
            if(!rule.test(this.form.name)) {
                this.$message.error('科室名称应为汉字、数字或字母且长度为2到20个字符!'); 
        		return;
            }
            ...
            if(!positionRule.test(this.form.position)) {
                this.$message.error('位置应为汉字、字母且长度为2到20个字符!'); 
        		return;
            }
            ...
            if(!useRule.test(this.form.use)) {
                this.$message.error('用途应为汉字、字母且长度为2到10个字符!'); 
        		return;
            }
