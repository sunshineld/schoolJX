        ...
        >
            ...
            <template slot="content" slot-scope="props">
                	...
                	:title="title" width="30%">
	                   <template slot="content">
							  ...
							  <el-form-item label="班	级:" :label-width="formLabelWidth">
							    <el-input v-model="form.class" placeholder="1 班"></el-input>
							  </el-form-item>
	                ...
	                title="删除" width="500px">
	                    <template slot="content">
	                        <span>删除班级的同时会删除对应的关联信息，确认删除吗？</span>
	                    </template>
	                    <template slot="foot">
	                        <el-button 
...
export default {
    ...
    methods: {
        ...
        //确定新增班级
        addClass() {
			let classRule = /^[0-9a-zA-Z\u4e00-\u9fa5]{2,10}$/;
        	if(this.form.grade == ""){
			...
			if(!classRule.test(this.form.class)) {
				this.$message.error('班级应为汉字、数字或字母且长度为2到10个字符！');
        		return;
			}
        	if(this.form.class_teacher == "" ){
        		this.$message.error('请选择班主任名字!');
        		return;
			}
			// console.log("--------",this.form.class_teacher)
			// var arr = this.form.class_teacher.split("(");
        	//提交
        	if(this.isAdd){
        		this.$http.post("/SchoolClass/add",{
					...
					class_teacher_id: this.form.class_teacher
				})
        		.then((res) => {
        ...
        editshow(val) {
			...
			for(let key in this.leaderData) {
				if(this.leaderData[key].search(val.class_teacher_name) != -1) {
					this.form.class_teacher= key;
				}
			}
