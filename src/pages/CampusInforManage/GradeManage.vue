        ...
        >
            ...
            <template slot="content" slot-scope="props">
	                   ...
	                   <template slot="content">
							 ...
							 <el-form-item label="教育阶段:" :label-width="formLabelWidth" class="el-label-text" >
							  ...
							  <el-form-item label="学        制:" :label-width="formLabelWidth">
							    <el-select v-model="formInline.school_length" placeholder="请选择" @change="schoolLengthChange">
							  ...
							  <el-form-item label="所在年级:" :label-width="formLabelWidth">
							    <el-input v-model="formInline.grade" placeholder="请输入所在年级"></el-input>
							  </el-form-item>
							  ...
							  <el-form-item label="年级主任:" :label-width="formLabelWidth">
							  	 <el-select v-model="formInline.grade_leader" placeholder="请选择" filterable>
							      <el-option v-for="(item,index) in leaderData" :label="item" :key="index"  :value="index"></el-option>
							    </el-select>
							  </el-form-item>
							</el-form>
	                    </template>
	                ...
	                title="删除" width="500px">
	                    <template slot="content">
	                        <span>删除年级的同时会删除对应的关联信息，确认删除吗？</span>
	                    </template>
	                    <template slot="foot">
	                        <el-button 
...
export default {
    ...
    methods: {
    	...
    	addGrade() {
			...
			let gradeRule = /^[0-9\u4e00-\u9fa5]{2,20}$/;
			let yearRule = /^[1-9]{1}[0-9]{3}$/;
			...
			if(!gradeRule.test(this.formInline.grade)) {
				this.$message.error('年级应为汉字或数字且长度为2到20个字符！');
    			return;
			}
			...
			if(!yearRule.test(this.formInline.grade_level)) {
				this.$message.error('请输入正确的入学年份');
				return;
			}
    		if(this.formInline.grade_leader == "") {
    		...
    		if(this.isAdd){
        		this.$http.post("/SchoolGrade/add",{
	    			...
	    			grade_leader: this.formInline.grade_leader
        		})
        		.then((res) => {
				...
				console.log("确定保存",this.formInline)
            	this.$http.post("/SchoolGrade/edit",{
            		token: localStorage.jxtoken,
	    			grade_type: this.formInline.grade_type,
	    			school_length: this.school_length,
	    			grade_level: this.formInline.grade_level,
	    			grade: this.formInline.grade,
	    			grade_leader: this.formInline.grade_leader,
        ...
        editshow(val) {
			this.title = "编辑年级";
			this.school_length = val.school_length;
        	console.log(1111,val);
        	for (var key in val) {
                this.formInline[key] = val[key];
            }
			this.getSchoolType(val.school_length);
			for(let key in this.leaderData) {
				if(this.leaderData[key].search(val.grade_leader_name) != -1) {
					this.formInline.grade_leader = key;
				}
			}
		...
		//编辑时学制数据变化
		schoolLengthChange() {
			this.school_length = this.formInline.school_length;
		},
        //获取教育阶段和学制
        getGradeType() {
        ...
        getLeaderName() {
        	...
        	.then(res => {
				...
				for(let i = 0; i < data.length; i++) {
					this.leaderData[data[i].id] = data[i].name + "(" + data[i].phone + ")";
				}
