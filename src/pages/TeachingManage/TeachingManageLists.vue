<template>
    <div class="teaching-manage-lists">
        <I-table
        title="教学任务生成"    
        :isUpdate="true"
        :isBack="false"
        @update="init">
            <template slot="head">
                    <el-select clearable v-model="qryForm.teaching_subject_id" filterable default-first-option placeholder="请选择教学科目" @change="selectChangeForSubject" style="width: 150px;">
                        <el-option v-for="item in subjectList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                    <el-select clearable v-model="qryForm.grade_id" filterable default-first-option placeholder="请选择年级" style="width: 150px;">
            ...
            <template slot="content" slot-scope="props">
                    ...
                    @selection-change="changeFun">
                        ...
                        <template slot-scope="scope">
                            <el-button  type="text" size="small" @click="btnClickForEdit(scope.row)"> 编辑</el-button>
                            <el-button type="text" size="small" @click="btnClickForDel(scope.row)" >删除</el-button>

                            <!-- <el-button  type="text" size="small" @click="btnClickForEdit(scope.row)" v-if="scope.row.status!='启用'"> 编辑</el-button>
                            <el-button  type="text" size="small" disabled v-if="scope.row.status=='启用'" style="background:#c1c1c1;"> 编辑</el-button>
                            <el-button type="text" size="small" @click="btnClickForDel(scope.row)" v-if="scope.row.status!='启用'">删除</el-button>
                            <el-button type="text" size="small"  disabled v-if="scope.row.status=='启用'"  style="background:#c1c1c1;">删除</el-button> -->
                        </template>
                        </el-table-column>
                    </el-table>
                <!-- 新增 -->
                ...
                title="删除">
                    <template slot="content">
                        <div class="outportaddr-group">是否确定删除已选中的任务？</div>
                    </template>
                    <template slot="foot">
            ...
            <template slot="foot">
              <el-button size="smail" type="danger" @click="deletefnbtn" style="margin-left:10px;">批量删除</el-button>
                <I-table-page
                :total="total"
                :pageSize="pageSize"
...
export default {
  ...
  methods: {
    init() {
      ...
      this.qryForm.status = "";
      this.qryForm.page_number = this.pageIndex;
      this.qryForm.page_size = this.pageSize;
        ...
        .then(res => {
          res.data.row.forEach(e => {
            ...
            for (let info in e.infos) {
              tmpObj.gradeList.push({
                label: e.infos[info].grade_name,
                value: e.infos[info].grade_id
              });
            }
    ...
    doSearch() {
      this.$http
        .post("/Teaching/taskLists", this.qryForm)
        .then(res => {
          console.log("res", res);
          ...
          if (
            this.pageIndex > this.total / this.pageSize &&
            this.pageIndex > 1
          ) {
            this.pageIndex -= 1;
          }
    ...
    selectChangeForSubject() {
      ...
      if (this.qryForm.teaching_subject_id == "") {
        this.gradeList = [];
        return;
      }
      this.gradeList = [];
    ...
    selectChangeForSubjectAddAndEdit() {
      ...
      this.subjectList.forEach(e => {
        if (e.value == this.addAndEditForm.teaching_subject_id) {
          e.gradeList.forEach(ec => {
            console.log(JSON.stringify(ec));
            if (ec.label == null) ec.label = "null";
            this.addAndEditForm.gradeList.push({
              label: ec.label,
              value: ec.value
            });
          });
      ...
      // if (this.addAndEditForm.gradeList.length > 0)
      //   this.addAndEditForm.grade_id = this.addAndEditForm.gradeList[0].value;
    },
    ...
    btnClickForEdit(obj) {
      if (obj.status == "启用") {
        this.$message.error("该任务已启用，请在学期结束后进行编辑！");
        return;
      }
      // console.log("obj",obj)
      this.subjectList.forEach(e => {
    ...
    btnClickForAddAndEditYes() {
      ...
      if (
        this.addAndEditForm.total_class_hour.trim() == "" ||
        this.addAndEditForm.total_class_hour.toString().trim() == ""
      ) {
        this.$message.error("请输入总课时数!");
        return;
      }
      if (
        this.addAndEditForm.teaching_week.trim() == "" ||
        this.addAndEditForm.teaching_week.toString().trim() == ""
      ) {
      ...
      if (
        this.addAndEditForm.total_class_hour < 0 ||
        this.addAndEditForm.total_class_hour > 500
      ) {
        this.$message.error("总课时必须为数字，并且不能超过500");
        return;
      }
      ...
      if (
        this.addAndEditForm.teaching_week < 0 ||
        this.addAndEditForm.teaching_week > 500
      ) {
        this.$message.error("周课时必须为数字，并且不能超过50");
        return;
      }
      this.$http
        .post("/Teaching/saveTask", {
