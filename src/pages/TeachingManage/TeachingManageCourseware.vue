<template>
    <div>
        ...
        @update="update">
            <template slot="head">
                    <el-input v-model="search" size="smail" placeholder="请选择课件名称" style="width:150px;"></el-input>
                    ...
                    <template slot="content">
                        ...
                        <div class="item">
                            <el-upload
                                ...
                                <span>(只能是PPT，pdf，word，excel，文件名请不要包含空格)</span>
                            </el-upload>
                        </div>
                    </template>
                    <template slot="foot">
  ...
  methods: {
        ...
        .then(res => {
          ...
          if (res.data.row) {
            res.data.row.forEach(item => {
              item.create_time = this.$tools.formatDate(
    ...
    handleCheck(file, fileList) {
      ...
      if (file.size > 104857600) {
        this.$message.error("上传课件不能大于100M");
        return false;
      }
    ...
    checkFn(val) {
      ...
      this.showAuthor = val.name;
      this.showCreateTime = this.$tools.formatDate(
        val.create_time.toString(),
        "yyyy-MM-dd"
      );
    ...
    loadFn(val) {
      this.$tools.exportFile("/Teaching/forceDownload", {
        token: localStorage.jxtoken,
        file_url_name: encodeURI(val.file_url) + encodeURI(val.file_name)
      });
