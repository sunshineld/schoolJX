...
class HTTP {
  ...
  _log(method, url, data) {
    console.log(
      `${HTTP.BASEURL + url}, method: ${method}, data: ${JSON.stringify(data)}`
    );
  }
    ...
    return HTTP.h
      ...
      .catch(obj => {
        ...
        if (obj.status === HTTP.RES_STATUS_FAIL) {
          ...
          //############### add by xing.wei 2018.03.26  start ####################
          // let from = router.history.current.name || "";
          // if(from != "Home"){
